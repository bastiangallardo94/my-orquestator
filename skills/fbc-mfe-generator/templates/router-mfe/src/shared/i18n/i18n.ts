...
const getCurrentLanguage = (): string => {
    try {
        ...
        console.error('[maint-maintainer] Could not get language, using default EN', error);
        return 'EN';
    }
};
...
i18n
            ...
            ({ configuration }: GlobalStoreState) => {
                if (configuration.language && configuration.language !== i18n.language) {
                    i18n.changeLanguage(configuration.language)
                        ...
                        .catch((error) => {
                            console.error(`❌ [maint-maintainer] Error changing language:`, error);
                        });
                }
            }
        );
    ...
    .catch((error) => {
        console.error('❌ [maint-maintainer] Error initializing i18n:', error);
    });
